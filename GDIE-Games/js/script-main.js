...
function botonSiguiente() {
    var tiempo = siguienteCue(cueIdx);
    if (tiempo != null) {
        video.currentTime = tiempo;
        cueIdx++;
        cueActual= video.textTracks[0].activeCues[cueIdx];
        $("#bt-siguiente").prop("disabled", false);
        $("#bt-anterior").prop("disabled", false);
    } 
    else 
    {
        $("#bt-siguiente").prop("disabled", true);
        if (anteriorCue(cueIdx) != null) $("#bt-anterior").prop("disabled", false);
    }
...
function botonAnterior() {
    ...
    {
        $("#bt-anterior").prop("disabled", true);
        if (siguienteCue(cueIdx) != null) $("#bt-siguiente").prop("disabled", false);
    }
...
function siguienteCue(numCueAct) {
    ...
    //alert(numCueAct+"  "+cues.length);
    if (numCueAct + 1 >= cues.length) {
        return null;
    }
    return cues[numCueAct+1].startTime;
}

function anteriorCue(numCueAct) {
    var cues = video.textTracks[0].cues;
    //si no quedan más cues ...
    //alert(numCueAct+"  "+cues.length);
    if (numCueAct - 1 < 0) {
...
function actualizaQuiz() {
    ...
    //$.getJSON("assets/quiz/preguntas.json", function (json) {
        ...
        if (cueActual != null) {
            ...
            //for (var i = 0; i < json.length; i++) {
                ...
                //if (json[i].id == info.pregunta) {
                    ...
                    respuestaCorrectaActual = info.respuestaCorrecta;
                    textoCorrecta = info.respuestas[respuestaCorrectaActual];
        ...
        $("#botonQuiz0").html("<font color='#FFFFFF'>"+respuestas[0]+"</font>");
        $("#botonQuiz1").html("<font color='#FFFFFF'>"+respuestas[1]+"</font>");
        $("#botonQuiz2").html("<font color='#FFFFFF'>"+respuestas[2]+"</font>");
        $("#botonQuiz3").html("<font color='#FFFFFF'>"+respuestas[3]+"</font>");


        if (hayPregunta) {
            $("#preguntas").html("<h5><font color='#FFFFFF'>"+pregunta+"</font></h5>");
        } else {
            $("#preguntas").html("");
            divRespuestas.innerHTML = "";
        }
        if (contestada) {
            $("#preguntas").html("<font color='#FFFFFF'>"+"Ya has contestado esta pregunta."+"</font>");
            divRespuestas.innerHTML = "";
        }

        $("#score").html("<font color='#FFFFFF'>"+"Aciertos: " + aciertos + "  Errores: " + errores + ""+"</font>");
    //});
}
...
function evaluarRespuesta(numRespuesta) {
    ...
    if (numRespuesta == respuestaCorrectaActual) {
        $("#respuestas").html("<font color='#00FF00'>"+"<b>Respuesta correcta! </b> <br>"+textoCorrecta+"</font>");
        aciertos += 1;
    } else {
        $("#respuestas").html("<font color='#FF0000'>"+"<b>Respuesta incorrecta! </b> La correcta es: <br>"+textoCorrecta+"</font>");
        errores += 1;
    }
    //Modificar score
    $("#score").html("<font color='#FFFFFF'>"+"Aciertos: " + aciertos + "  Errores: " + errores + ""+"</font>");
