...
function cargarVideo(path) {
    ...
    // Mostrar aviso
    var descr = "El vídeo y las marcas temporales se han cargado correctamente.";
    if ($("#file-selector").val() == null) {
        descr = "El vídeo se ha subido al servidor.";
    }
    crearAviso("alert-success", "", descr, 5500);
...
function updateDatos(cue) {
    ...
    if (inicioPulsado) {
        ...
        crearAviso("alert-danger", "Cuidado:", descr, 0);
        if ($("#md-fin").val() == "") {
            botonFin();
        }
        return;
...
//alert("actualizo");
    ...
    $("#md-pregunta").val(info.pregunta);
    $("#md-respuesta1").val(info.respuestas[0]);
    $("#md-respuesta2").val(info.respuestas[1]);
    $("#md-respuesta3").val(info.respuestas[2]);
    $("#md-respuesta4").val(info.respuestas[3]);
    $("#md-solucion").val(info.respuestaCorrecta);
...
function peticionSubirVideo() {
    ...
    if ((!name.includes(".mp4")) && (!name.includes(".webm"))) {
        var descr = "El vídeo seleccionado no tiene el formato adecuado (.mp4).";
    ...
    //alert("Creando aviso");
    crearAviso("alert-info", "", "Se está subiendo el archivo. Espera por favor.", 4000);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            // Si ya existe un video con el mismo nombre mostrar aviso
                ...
                crearAviso("alert-danger", "Cuidado:", descr, 4000);
            }
            else {
...
function peticionSubirMetadatos() {
    ...
    $.post("php/uploadMetadata.php", {
        ...
        .done(function (data) {
            {
                // Crear aviso
                var descr = "La información se han guardado en el servidor."
                crearAviso("alert-success", "", descr, 4000);
...
function crearAviso(tipo, titulo, descr, tiempo) {
    ...
    var miss = document.getElementById("alert");
    var codi;

    codi="";
    if (tipo=="alert-success") codi+="<span style='color: green;'>"
    if (tipo=="alert-danger") codi+="<span style='color: red;'>"
    if (tipo=="alert-info") codi+="<span style='color: cyan;'>"
    codi+="<left><strong>" + titulo + " </strong>" + descr+"</left>";
    codi+="</span>";
    miss.innerHTML = codi;
